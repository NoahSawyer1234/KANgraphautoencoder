...
# of the graph laplacian (topological info) concatenated with the average node vector (post-node embedding, 
...
def compute_lap_eigenvalues_with_graph(g, k=10):
...
def pre_process_lap_eigenvectors(graph_list, k=10):
    return [compute_lap_eigenvalues_with_graph(g, k=k) for g in graph_list]

class GraphEigenvalueDataset(Dataset):
    def __init__(self, graph_list, eigval_list, label_list):
    ...
    def __getitem__(self, index):
        return self.graphs[index], self.eigvals[index], self.labels[index]
...
if __name__ == '__main__':
    ...
    train_graph_eigvals = pre_process_lap_eigenvectors(train_graphs)
    valid_graph_eigvals = pre_process_lap_eigenvectors(valid_graphs)
    test_graph_eigvals  = pre_process_lap_eigenvectors(test_graphs)
    # Need to unzip for batches
    train_gs, train_evs = zip(*train_graph_eigvals)
    valid_gs, valid_evs = zip(*valid_graph_eigvals)
    test_gs, test_evs = zip(*test_graph_eigvals)
    ...
    train_loader = DataLoader(
        GraphEigenvalueDataset(train_gs, train_evs, train_labels), 
    ...
    valid_loader = DataLoader(
        GraphEigenvalueDataset(valid_gs, valid_evs, valid_labels), 
        batch_size=int(state['batch_size']), 
    ...
    test_loader = DataLoader(
        GraphEigenvalueDataset(test_gs, test_evs, test_labels), 
        batch_size=int(state['batch_size']), 
